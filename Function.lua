		...
		for playerName, playerData in pairs(addonCache) do
			...
			if not currentPlayers[playerName] then
				if playerName == "Entidad desconocida" then
					...
					SendSystemMessage(
						playerData.class .. ". Roles liberados: " .. table.concat(getPlayerRoles(playerData.rol), ", ")
					)
				end
...
StaticPopupDialogs["PULL_TIMER_INPUT"] = {
	...
	OnShow = function(self)
		...
		self.editBox:SetNumber(10) -- Valor por defecto
		self.editBox:SetFocus()
		self.editBox:HighlightText()
	end,
	OnAccept = function(self)
		local seconds = tonumber(self.editBox:GetNumber())
		if seconds and seconds > 0 then
			-- Ejecutar el comando de DBM para iniciar la cuenta regresiva
			SendChatMessage("SOLO EL TANQUE PULEA", "RAID_WARNING")
			local dbmAvailable = SafeDBMCommand("broadcast timer 0:"..seconds.." RESPETAR PULL")
			if dbmAvailable then
				SafeDBMCommand("pull "..seconds)
			else
				SendSystemMessage(
					"|cFFFFFF00Advertencia:|r DBM no está instalado. La función de temporizador no está disponible."
				)
...
StaticPopupDialogs["CONFIRM_REMOVE_BLACKLIST"] = {
	...
	OnShow = function(self)
		...
		self.text:SetFormattedText(
			"¿Estás seguro de que deseas eliminar a %s de la lista negra?\nRazón: %s",
			self.targetName,
			reason
		)
	end,
...
function AssignIconsAndAlert(button)
	...
	for playerName, playerData in pairs(addonCache) do
		...
		if #playerRoles > 0 then
			local rolesString = #playerRoles == 1 and playerRoles[1]
				or table.concat(playerRoles, ", ", 1, #playerRoles - 1) .. " y " .. playerRoles[#playerRoles]
...
function showTargetInfo()
	...
	-- Línea 2: Salud
	local line2 = string.format("Salud: %s/%s [%d%%]", FormatNumber(health), FormatNumber(healthMax), healthPct)
	...
	if powerType == 0 then -- MANA
		...
		if manaMax > 0 then
			...
			line2 = line2
				.. string.format(" //  Mana: %s/%s [%d%%]", FormatNumber(mana), FormatNumber(manaMax), manaPct)
		end
	elseif powerType == 3 then -- ENERGÍA
		...
		line2 = line2 .. string.format(" //  Energia: %d/%d [%d%%]", energy, energyMax, energyPct)
	elseif powerType == 1 then -- IRA
		...
		line2 = line2 .. string.format(" // Ira: %d/%d [%d%%]", rage, rageMax, ragePct)
	end
		...
		if UnitExists(unit) and not UnitIsUnit(unit, "player") then
			...
			if name then
				...
				if statusTag then
					table.insert(
						flaggedMembers,
						name .. (class and (" (" .. class .. ")") or "") .. " [" .. statusTag .. "]"
					)
				end
			end
		end
...
function checkTempleGear(playerName)
	...
	if raidInfo[playerName] then
		...
		if hasTempleGear and isChecked then
			...
			SendChatMessage(
				"Tienes (" .. counter .. ") parte" .. (counter <= 1 and "" or "s") .. " PVP",
				"WHISPER",
				nil,
				playerName
			)
				...
				for _, slot in ipairs(slots) do
					...
					if slotId then
						...
						if itemLink then
							...
							if
								string.find(string.lower(itemName), "incansable")
								or string.find(string.lower(itemName), "colérico")
								or string.find(string.lower(itemName), "furioso")
							then
								hasPvP = true
								break
							end
...
function PerformGuildRoulette()
	...
	-- Obtener miembros en línea de los rangos 2 y 3 (rankIndex 1 y 2 ya que es 0-based)
	local onlineMembers = GetOnlineGuildMembers({ 3, 4,5 })

	if #onlineMembers == 0 then
		SendSystemMessage("No hay miembros de la hermandad de los rangos 2 y 3 conectados para el sorteo.")
		return
	end

	-- Seleccionar 5 jugadores aleatorios con sus rangos
	local selectedPlayers = {}
	if #onlineMembers <= 5 then
		-- Si hay 5 o menos jugadores, tomarlos a todos con sus rangos
		...
		-- Si hay más de 5, seleccionar 5 al azar con sus rangos
		local tempTable = {}
		for i = 1, #onlineMembers do
			local name = onlineMembers[i]
			local rankName = GetGuildMemberRank(name)
	...
	local messages = {
		...
		"El premio aleatorio de este sorteo es: " .. itemText,
		"Participan hasta 5 jugadores de los rangos Iniciado y superiores en linea:",
		"",
	}
	...
	-- Generar puntajes para los jugadores seleccionados
	local scores = {}
	local maxScore = 0
	local winnerIndex = 1
		...
		if idx == winnerIndex then
			table.insert(
				...
				"["
					...
					.. "] "
					.. member.name
					.. " ha obtenido el mayor puntaje con "
					.. scores[idx]
					.. " puntos!"
			)
		else
			table.insert(messages, "[" .. member.rank .. "] " .. member.name .. " ha obtenido " .. scores[idx] .. " puntos.")
		end
	end
	...
	table.insert(messages, "¡El ganador es [" .. winner.rank .. "] " .. winner.name .. "! Ha ganado " .. itemText)
	...
	SendChatMessage(
		"¡Ganaste el sorteo! Tu premio es: "
			.. selectedGuildBankItem.link
			.. " (x"
			.. selectedGuildBankItem.count
			.. "). Si es util para alguno de tus personajes reclama el premio a un GM/Alter en el Banco Alianza de Dalaran o solicita por el chat de hermandad que se envie a tu correo.",
		"WHISPER",
		nil,
		winner.name
	)
	...
	-- SendSystemMessage(
	--     "¡Ha ganado el sorteo! Su premio es: " .. selectedGuildBankItem.link ..
	--     " (x" .. selectedGuildBankItem.count .. "). Por favor, contacte a un GM/Alter para reclamar su premio o se guardara para un nuevo sorteo.",
	--     "WHISPER", nil, winner.name)
...
function CheckGuildOfficerNotes()
	...
	if not encontrados then
		...
		table.insert(
			mensajes,
			"NIVELES: 1.Recluta 2.Élite 3.Comandante // RECONOCIMIENTOS: ESPÍRITU, SABIO, CAZADOR, DOMINION, GUARDIÁN"
		)
		...
		local function agregarLineaJugadores(jugadores, inicio, fin)
			...
			for i = inicio, math.min(fin, #jugadores) do
				...
				table.insert(
					linea,
					string.format("%s %s (%s)", j.clase, j.nombre, table.concat(j.reconocimientos, ", "))
				)
			end
			if #linea > 0 then
...
function ShowGuildHierarchy(channel)
	if not IsInGuild() then
		SendSystemMessage("No eres miembro de una hermandad.")
		return
	end
	...
	if #rankNames >= 2 then
		...
		if count1 > 0 or count2 > 0 then
			table.insert(
				messages,
				string.format("→ %s [%d] + %s [%d] = Maestro de la hermandad", rank1, count1, rank2, count2)
			)
		end
		...
		for rankIndex = 3, #rankNames do
			...
			if total > 0 then
				local description = ""
				if rankIndex == 3 then
					description = "Organizadores, ayudantes y moderadores"
				elseif rankIndex == 4 then
					description = "Asistencia a raid confiable"
				elseif rankIndex == 5 then
					description = "Funcion y GS claros"
				elseif rankIndex == 6 then
					description = "Deben enviar su información al GM/Alter/Oficial para ser ascendidos"
				end
	...
	table.insert(
		messages,
		string.format("Total: %d miembros (%d en línea, %d desconectados)", totalMembers, totalOnline, totalOffline)
	)
	...
	if channel == "SYSTEM" then
		SendSystemMessage(table.concat(messages, "\n"))
	else
		SendDelayedMessages(messages, "GUILD")
	end
end
