...
function RaidDominion:HandleMainOption(option, button)
	...
	elseif option == "Auras" then
		self:ShowMenu(self.aurasMenu, self.aurasMenuWidth, self.aurasMenuHeight)
...
function RaidDominion:HandleGuildOption(option)
	...
	elseif option == "Jerarquia" then
		if not isTopTwoRanks() then
			ShowGuildHierarchy("SYSTEM")
		else
			ShowGuildHierarchy()
		end
	end
...
function RaidDominion:ShowGuildRulesMenu()
	...
	if not self.guildRulesMenu then
		self.guildRulesMenu, self.guildRulesMenuWidth, self.guildRulesMenuHeight = CreateMenu(
			self.frame,
			rulesList,
			-12,
			function(rule)
...
function RaidDominion:ShowRulesMenu()
	...
	if not self.rulesMenu then
		self.rulesMenu, self.rulesMenuWidth, self.rulesMenuHeight = CreateMenu(
			self.frame,
			rulesList,
			-12,
			function(rule)
...
function RaidDominion:ShowMechanicsMenu()
	...
	if not self.mechanicsMenu then
		self.mechanicsMenu, self.mechanicsMenuWidth, self.mechanicsMenuHeight = CreateMenu(
			self.frame,
			mechanicsList,
			-12,
			function(mechanic)
...
function RaidDominion:Init()
	...
	self.primaryMenu, self.primaryMenuWidth, self.primaryMenuHeight = CreateMenu(
		self.frame,
		primaryRoles,
		-12,
		function(role)
	...
	self.primarySkillsMenu, self.primarySkillsMenuWidth, self.primarySkillsMenuHeight = CreateMenu(
		self.frame,
		primarySkills,
		-12,
		function(role)
	...
	self.secondaryMenu, self.secondaryMenuWidth, self.secondaryMenuHeight = CreateMenu(
		self.frame,
		primaryBuffs,
		-12,
		function(role)
	...
	self.secondaryRolesMenu, self.secondaryRolesMenuWidth, self.secondaryRolesMenuHeight = CreateMenu(
		self.frame,
		secondaryRoles,
		-12,
		function(role)
	...
	self.aurasMenu, self.aurasMenuWidth, self.aurasMenuHeight = CreateMenu(self.frame, auras, -12, function(role)
		RaidDominion:HandleAssignableRole(role)
	end, true, "Aura")
	self.addonMenu, self.addonMenuWidth, self.addonMenuHeight = CreateMenu(
		self.frame,
		addonOptions,
		-12,
		function(option)
	...
	self.guildMenu, self.guildMenuWidth, self.guildMenuHeight = CreateMenu(
		self.frame,
		guildOptions,
		-12,
		function(option)
	...
	for i = 1, 3 do
		...
		if btn and not btn.hooked then
			...
			btn:SetScript("OnMouseUp", function(self)
				local hasMain, mainMs, mainCharges, hasOff, offMs, offCharges, hasRanged, rangedMs, rangedCharges =
...
f:SetScript("OnEvent", function(self, event, arg1)
	...
	if event == "PLAYER_ENTERING_WORLD" or event == "UNIT_INVENTORY_CHANGED" or event == "PLAYER_EQUIPMENT_CHANGED" then
